...
namespace Controladores
{
    public class PisoControlador : IControlador<Piso>
    {
        ...
        public async Task<Piso> add(Piso tareaNueva)
        {
            ...
            HttpResponseMessage mensaje = await cliente.PostAsync("http://10.203.31.200:8080/api/pisos", content);
        ...
        public async Task<List<Piso>> getAll()
        {
            try
            {
                ...
                HttpResponseMessage mensaje = await cliente.GetAsync("http://10.203.31.200:8080/api/pisos");
        ...
        {
            HttpResponseMessage mensaje = await cliente.DeleteAsync($"http://10.203.31.200:8080/api/pisos/{id}");
            mensaje.EnsureSuccessStatusCode();
            return true;
        }
        ...
        public async Task<Piso> update(Piso tareaModificada, int id)
        {
            ...
            HttpResponseMessage mensaje = await cliente.PutAsync($"http://10.203.31.200:8080/api/pisos/{id}", content);
            mensaje.EnsureSuccessStatusCode();
